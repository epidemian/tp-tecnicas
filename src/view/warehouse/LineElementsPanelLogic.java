...
public class LineElementsPanelLogic {
    ...
    public LineElementsPanelLogic(List<ProductionLineElement> lineElements,
            List<LineElementButton> lineElementButtons, JButton previousMachines,
            JButton nextMachines) {
        ...
        for (final LineElementButton button : this.lineElementButtons){
            button.addActionListener(new ActionListener() {
                ...
                /*
                 * Sets the lineElement store in this class with the one that
                 * is contained in the button.
                 */
                @Override
                public void actionPerformed(ActionEvent ae) {
                   LineElementsPanelLogic.this
                   .setProductionLineElement(button.getProductionLineElement());
                }
            });
        }
    }
    ...
    private void setUpLineElementButtons(){
        ...
        int lastIndex = this.lineElements.size() >= startIndex + this.lineElementButtonsSize ?
                        startIndex + this.lineElementButtonsSize:
                        this.lineElements.size();
         ...
         for (int i = startIndex; i < lastIndex; i++, j++)
              this.lineElementButtons.get(j).setProductionLineElement(this.lineElements.get(i));
